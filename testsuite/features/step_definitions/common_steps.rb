...
Then(/^I should have '([^']*)' in the metadata for "([^"]*)"$/) do |text, host|
  ...
  target = $client
  cmd = "zgrep '#{text}' #{client_raw_repodata_dir("test-channel-for-sle")}/*primary.xml.gz"
...
Then(/^"([^"]*)" should exist in the metadata for "([^"]*)"$/) do |file, host|
  raise 'Invalid target.' unless host == 'sle_client'
  node = $client
  dir_file = client_raw_repodata_dir("test-channel-for-sle")
...
Then(/^I should have '([^']*)' in the patch metadata$/) do |text|
  cmd = "zgrep '#{text}' #{client_raw_repodata_dir("test-channel-for-sle")}/*updateinfo.xml.gz"
